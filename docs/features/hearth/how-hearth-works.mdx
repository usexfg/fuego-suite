...
fee   = gross − paid                        // debited from LP reserves → CD accumulator
```

The fee accrues in a HEAT-denominated accumulator (`cdHearthFeeAccumulator`) and is minted into the CD yield vault (`CD_APY_POOL`) at each epoch boundary; the consumed XFG side is burned 50/50 (Eternal Flame / SWF). There is **no** separate “0.3% Hearth LP fee”.
