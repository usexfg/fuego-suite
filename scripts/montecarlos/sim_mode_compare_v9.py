...
def xfg_price_model(t, rng):
    """XFG flat at $2 ± noise (no drift, ideal conditions)"""
    return max(0.50, 2.0 * float(1.0 + rng.normal(0.0, 0.15)))
...
def sim_core(rng, launch, threshold, activation_mode):
    ...
    # Mode 4 / Mode 5: bootstrap pool at target ratio
    if activation_mode == 'fixed_peg' or activation_mode == 'no_pi':
    ...
    for ep in range(N_EPOCHS):
        ...
        elif activation_mode == 'no_pi':
            # MODE 5: no PI, no peg arb, no target — current implementation
            # Pool ratio floats freely. No redemption price target.
            # Fees → CD yield (buy HEAT from pool) + treasury.
            target = spot  # just use spot for tracking

        target = max(0.00001, target)

        # ── PI Controller (skipped for fixed_peg and no_pi) ──
        if activation_mode == 'fixed_peg' or activation_mode == 'no_pi':
        ...
        # ── Arbitrage (Mode 4 only; skipped for no_pi) ──
        ...
        # User mint
        mp = max(0.0, 0.008 - 0.4 * (abs(dev) if activation_mode != 'fixed_peg' and activation_mode != 'no_pi' else peg_dev))
        ...
        # Treasury rebalancer (skipped for fixed_peg and no_pi)
        if (activation_mode != 'fixed_peg' and activation_mode != 'no_pi' and
...
MODELS = OrderedDict([
    ...
    ('I: No PI',      dict(launch=LAUNCH_8, threshold=0,          mode='no_pi')),
])
...
print(f"  XFG price model: flat $2 (no drift, 15% noise) — ideal conditions")
print(f"  G = MODE 2 (current old): 8:1 self-ref × CPI  |  H = MODE 4: fixed $1.58 peg")
print(f"  I = MODE 5 (NEW): no PI, no peg arb, no target — current implementation")
print(f"{'='*120}")
