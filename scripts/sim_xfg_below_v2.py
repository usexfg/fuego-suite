#!/usr/bin/env python3
"""XFG stress test v2 — fixed: no can_mint gate. Two-way arb always."""
...
def run(rng,price_fn):
    ...
    for ep in range(N_EPOCHS):
        ...
        # Two-way arb — ALWAYS active, no can_mint gate
        if sc<EPY and lo>0:
            for _ in range(40):
                s2=px/max(ph,1e-3);hp2=s2*ema;gap=abs(hp2-1.58)/1.58
                if gap<0.0005:break
                a=min(px,ph)*.03
                if hp2>1.58:  # mint + sell to pool
        ...
        # Mint always available (dollar-neutral)
        ...
        # Treasury absorption when pool is cheap
        ...
        if ep>0 and ep%EPY==0:
            if ca>0 and ph>0:hb=ph*ca/(px+ca)
...
print(f"\n{'='*95}  XFG BELOW PEG — FIXED (no mint gate)")
