// Copyright (c) 2017-2026 Fuego Developers
//
// This file is part of Fuego.
//
// Fuego is free software distributed in the hope that it
// will be useful, but WITHOUT ANY WARRANTY; without even the
// implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
// PURPOSE. See file labeled LICENSE for more details.

#include "Common/FixedPoint.h"

namespace CryptoNote {

static const uint64_t MASK64 = (uint64_t)-1;

FixedPoint64 FixedPoint64::fromUint64(uint64_t v) {
  return FixedPoint64((int128_t)v << 64, Raw{});
}

FixedPoint64 FixedPoint64::fromRatio(uint64_t num, uint64_t denom) {
  if (denom == 0)
    return FixedPoint64::zero();
  // (num / denom) in Q64.64 = (num << 64) / denom
  int128_t val = ((int128_t)num << 64) / (int128_t)denom;
  return FixedPoint64(val, Raw{});
}

uint64_t FixedPoint64::toUint64() const {
  return (uint64_t)(m_value >> 64);
}

FixedPoint64 FixedPoint64::add(FixedPoint64 o) const {
  return FixedPoint64(m_value + o.m_value, Raw{});
}

FixedPoint64 FixedPoint64::sub(FixedPoint64 o) const {
  return FixedPoint64(m_value - o.m_value, Raw{});
}

FixedPoint64 FixedPoint64::mul(FixedPoint64 o) const {
  // Compute (a * b) >> 64 in Q64.64 without 256-bit overflow.
  // Decompose each operand into signed hi and unsigned lo 64-bit halves:
  //   a = a_hi * 2^64 + a_lo
  //   b = b_hi * 2^64 + b_lo
  // Then (a * b) >> 64 = a_hi*b_hi*2^64 + a_hi*b_lo + a_lo*b_hi + (a_lo*b_lo)>>64
  //
  int64_t  a_hi = (int64_t)(m_value >> 64);
  uint64_t a_lo = (uint64_t)(m_value - ((int128_t)a_hi << 64));
  int64_t  b_hi = (int64_t)(o.m_value >> 64);
  uint64_t b_lo = (uint64_t)(o.m_value - ((int128_t)b_hi << 64));

  int128_t result = ((int128_t)a_hi * b_hi) << 64;   // hi*hi, shifted up
  result += (int128_t)a_hi * b_lo;                    // hi*lo
  result += (int128_t)a_lo * b_hi;                    // lo*hi
  result += (int128_t)(((uint128_t)a_lo * b_lo) >> 64); // lo*lo, shifted down

  return FixedPoint64(result, Raw{});
}

uint64_t FixedPoint64::mulToUint64(uint64_t v) const {
  // (this * v) in integer terms: (m_value * v) >> 64
  // Fits in int128_t for typical use (|this| up to a few *2^64, v up to MONEY_SUPPLY)
  return (uint64_t)((int128_t)(m_value * (int128_t)v) >> 64);
}

FixedPoint64 FixedPoint64::div(FixedPoint64 o) const {
  // Compute a / b in Q64.64: (a * 2^64) / b.
  // Use quotient-remainder decomposition to avoid 192-bit intermediate:
  //   q = a / b          (integer quotient)
  //   r = a % b          (remainder, |r| < |b|)
  //   a/b = q + r/b  →  result = q*2^64 + (r*2^64)/b
  // Since |r| < |b|, (r*2^64)/b fits in int128_t.
  //
  if (o.m_value == 0)
    return zero();

  int128_t q = m_value / o.m_value;
  int128_t r = m_value % o.m_value;

  int128_t result = (q << 64) + ((r << 64) / o.m_value);
  return FixedPoint64(result, Raw{});
}

FixedPoint64 FixedPoint64::negate() const {
  return FixedPoint64(-m_value, Raw{});
}

// ---------------------------------------------------------------------------
//  exp_approx — Taylor series for e^x, 8 terms
//    e^x = 1 + x + x²/2! + x³/3! + … + x⁷/7!
//    Valid for |x| ≤ 2.0 (about 1% error at bounds)
// ---------------------------------------------------------------------------
FixedPoint64 FixedPoint64::exp_approx(FixedPoint64 x) {
  // Clamp to prevent divergence
  int128_t two = (int128_t)2 << 64;
  if (x.m_value > two)  x.m_value = two;
  if (x.m_value < -two) x.m_value = -two;

  FixedPoint64 sum   = one();
  FixedPoint64 term  = one();       // term = x^k / k!
  FixedPoint64 xpow  = one();       // xpow = x^k

  for (int k = 1; k <= 8; ++k) {
    // xpow = x * xpow
    xpow = x.mul(xpow);
    // term = xpow / k!
    // k! for k=1..8: 1,2,6,24,120,720,5040,40320
    static const uint64_t fact[9] = {1, 1, 2, 6, 24, 120, 720, 5040, 40320};
    term = xpow.div(fromUint64(fact[k]));
    sum  = sum.add(term);
  }
  return sum;
}

// ---------------------------------------------------------------------------
//  ln_approx — Taylor series for ln(1 + y), 8 terms
//    ln(1+y) = y - y²/2 + y³/3 - y⁴/4 + …   for |y| < 1
//    Uses y = x - 1
// ---------------------------------------------------------------------------
FixedPoint64 FixedPoint64::ln_approx(FixedPoint64 x) {
  if (x.m_value <= 0)
    return zero(); // ln(0) = -∞, ln(negative) undefined

  FixedPoint64 y   = x.sub(one());       // y = x - 1
  FixedPoint64 sum = y;                  // first term
  FixedPoint64 ypow = y;                 // y^1
  int128_t     sign = -ONE;              // alternating sign: -1 for term 2, +1 for term 3, etc.

  for (int k = 2; k <= 8; ++k) {
    ypow = ypow.mul(y);                  // y^k
    // term = (sign * y^k) / k
    FixedPoint64 k_fp = fromUint64((uint64_t)k);
    FixedPoint64 term = ypow.div(k_fp);
    if (sign > 0)
      sum = sum.add(term);
    else
      sum = sum.sub(term);
    sign = -sign;
  }
  return sum;
}

} // namespace CryptoNote
