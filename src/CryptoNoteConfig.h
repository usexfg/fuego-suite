...
namespace CryptoNote
{
	namespace parameters
	{
		...
		const uint32_t DEPOSIT_MIN_TERM_v1 = 5480;  //blocks
                ...
                const uint32_t TESTNET_EPOCH_DURATION_BLOCKS = 10;  // fast test cycling
                const uint32_t SWAP_FEE_RATE_BPS = 100;             // 1% swap fee
                const uint32_t SWAP_FEE_RATE_DIVISOR = 10000;
                const uint32_t SWAP_FEE_CD_SHARE_PCT = 80;          // 80% to CD pool
                const uint32_t SWAP_FEE_TREASURY_SHARE_PCT = 20;    // 20% to treasury

                // PI Controller (hard-coded at launch)
                ...
                const uint64_t PI_MAX_RATE_NUM = 50;       // max ±50% annual
	...
	const uint8_t  BLOCK_MAJOR_VERSION_10                         = 10; // HEAT colored coin
