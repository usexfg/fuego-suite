...
void TransactionPrefixImpl::getOutput(size_t index, TransactionOutputOrder& output, uint64_t& amount) const {
  const auto& out = getOutputChecked(m_txPrefix, index, TransactionTypes::OutputType::Order);
