...
class BtcRpcClient {
public:
  BtcRpcClient(const std::string& host, uint16_t port,
               const std::string& rpcUser, const std::string& rpcPassword);

  // Basic queries
  bool getBlockCount(uint64_t& height);
  bool getTransaction(const std::string& txid, BtcTxInfo& info);
  bool getBalance(const std::string& address, uint64_t& satoshis);

  // UTXO queries
  bool listUnspent(const std::string& address, std::vector<BtcUtxo>& utxos);
  ...
  // Claim BTC from an HTLC by revealing the preimage (adaptor secret).
  ...
  // destAddress:    where to send the claimed BTC.
  ...
  // Refund BTC from an HTLC after the timeout has elapsed.
  // Uses P2WSH SegWit spending with witness stack.
  ...
  // destAddress:    where to send the refunded BTC.
