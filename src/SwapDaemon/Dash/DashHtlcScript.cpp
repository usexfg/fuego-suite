...
std::vector<uint8_t> DashHtlcScript::sha256(const std::vector<uint8_t>& data) {
  std::vector<uint8_t> digest(SHA256_DIGEST_LENGTH);
  SHA256(data.data(), data.size(), digest.data());
  return digest;
}

std::vector<uint8_t> DashHtlcScript::doubleSha256(const std::vector<uint8_t>& data) {
...
std::vector<uint8_t> DashHtlcScript::hexToBytes(const std::string& hex) {
  if (hex.size() % 2 != 0) {
...
std::string DashHtlcScript::bytesToHex(const std::vector<uint8_t>& bytes) {
...
void DashHtlcScript::writeLE16(std::vector<uint8_t>& out, uint16_t v) {
  out.push_back(static_cast<uint8_t>(v & 0xFF));
  out.push_back(static_cast<uint8_t>((v >> 8) & 0xFF));
}

void DashHtlcScript::writeLE32(std::vector<uint8_t>& out, uint32_t v) {
  out.push_back(static_cast<uint8_t>(v & 0xFF));
  out.push_back(static_cast<uint8_t>((v >> 8) & 0xFF));
...
void DashHtlcScript::writeLE64(std::vector<uint8_t>& out, uint64_t v) {
  for (int i = 0; i < 8; ++i) {
...
void DashHtlcScript::writeVarInt(std::vector<uint8_t>& out, uint64_t n) {
  if (n < 0xFD) {
...
void DashHtlcScript::pushData(std::vector<uint8_t>& script, const std::vector<uint8_t>& data) {
  size_t len = data.size();
  if (len == 0) {
    script.push_back(DashOpCode::OP_FALSE);  // OP_0 pushes empty byte vector
  } else if (len <= 75) {
    script.push_back(static_cast<uint8_t>(len));
    script.insert(script.end(), data.begin(), data.end());
  } else if (len <= 255) {
    script.push_back(DashOpCode::OP_PUSHDATA1);
    script.push_back(static_cast<uint8_t>(len));
    script.insert(script.end(), data.begin(), data.end());
  } else {
...
std::vector<uint8_t> DashHtlcScript::serializeScriptNum(uint32_t n) {
  if (n == 0) {
    return {};
  }
...
static const char kDashBase58Alphabet[] = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";

std::string DashHtlcScript::base58CheckEncode(uint8_t version, const std::vector<uint8_t>& payload) {
  ...
  while (!input.empty()) {
    ...
    encoded.push_back(kDashBase58Alphabet[remainder]);
...
bool DashHtlcScript::base58CheckDecode(const std::string& encoded, uint8_t& version,
  ...
  for (int i = 0; i < 58; ++i) {
    b58map[static_cast<uint8_t>(kDashBase58Alphabet[i])] = static_cast<int8_t>(i);
  }
...
bool DashHtlcScript::decodeAddress(const std::string& address, uint8_t& version,
...
std::vector<uint8_t> DashHtlcScript::buildP2pkhScriptPubKey(const std::vector<uint8_t>& pubKeyHash) {
  std::vector<uint8_t> script;
  script.reserve(25);
  script.push_back(DashOpCode::OP_DUP);
  script.push_back(DashOpCode::OP_HASH160);
  script.push_back(0x14);
  script.insert(script.end(), pubKeyHash.begin(), pubKeyHash.end());
  script.push_back(DashOpCode::OP_EQUALVERIFY);
  script.push_back(DashOpCode::OP_CHECKSIG);
  return script;
}

std::vector<uint8_t> DashHtlcScript::buildP2shScriptPubKey(const std::vector<uint8_t>& scriptHash) {
  std::vector<uint8_t> script;
  script.reserve(23);
  script.push_back(DashOpCode::OP_HASH160);
  script.push_back(0x14);
  script.insert(script.end(), scriptHash.begin(), scriptHash.end());
  script.push_back(DashOpCode::OP_EQUAL);
  return script;
}
...
std::vector<uint8_t> DashHtlcScript::createRedeemScript(
  ...
  script.push_back(DashOpCode::OP_IF);
  script.push_back(DashOpCode::OP_SHA256);
  pushData(script, hashLockSha256);
  script.push_back(DashOpCode::OP_EQUALVERIFY);
  pushData(script, recipientPubKey);
  script.push_back(DashOpCode::OP_CHECKSIG);
  script.push_back(DashOpCode::OP_ELSE);
  std::vector<uint8_t> lockTimeBytes = serializeScriptNum(timeoutBlock);
  pushData(script, lockTimeBytes);
  script.push_back(DashOpCode::OP_CHECKLOCKTIMEVERIFY);
  script.push_back(DashOpCode::OP_DROP);
  pushData(script, senderPubKey);
  script.push_back(DashOpCode::OP_CHECKSIG);
  script.push_back(DashOpCode::OP_ENDIF);
...
std::string DashHtlcScript::computeP2shAddress(const std::vector<uint8_t>& redeemScript, bool testnet) {
  std::vector<uint8_t> scriptHash = hash160(redeemScript);
  uint8_t version = testnet ? 0x13 : 0x10;
...
std::vector<uint8_t> DashHtlcScript::createClaimScriptSig(
  ...
  scriptSig.push_back(DashOpCode::OP_TRUE);
  pushData(scriptSig, redeemScript);

  return scriptSig;
}

std::vector<uint8_t> DashHtlcScript::createRefundScriptSig(
    const std::vector<uint8_t>& signature,
    const std::vector<uint8_t>& redeemScript) {
  std::vector<uint8_t> scriptSig;
  ...
  pushData(scriptSig, signature);
  scriptSig.push_back(DashOpCode::OP_FALSE);
  pushData(scriptSig, redeemScript);
...
std::vector<uint8_t> DashHtlcScript::buildRawTransaction(
  ...
  if (addrVersion == 0x4C || addrVersion == 0x8C) {
    // P2PKH (mainnet 0x4C, testnet 0x8C)
    scriptPubKey = buildP2pkhScriptPubKey(addrHash);
  } else if (addrVersion == 0x10 || addrVersion == 0x13) {
    // P2SH (mainnet 0x10, testnet 0x13)
...
std::vector<uint8_t> DashHtlcScript::redeemScriptToP2shScriptPubKey(
    const std::vector<uint8_t>& redeemScript) {
...
static bool dashReadVarInt(const uint8_t*& p, const uint8_t* end, uint64_t& out) {
...
std::vector<uint8_t> DashHtlcScript::parseClaimPreimage(
    const std::vector<uint8_t>& rawTx,
    const std::vector<uint8_t>& htlcP2shScriptPubKey) {

  if (htlcP2shScriptPubKey.size() != 23) return {};
  if (htlcP2shScriptPubKey[0] != DashOpCode::OP_HASH160) return {};
  if (htlcP2shScriptPubKey[1] != 0x14) return {};
  if (htlcP2shScriptPubKey[22] != DashOpCode::OP_EQUAL) return {};
  ...
  uint64_t vinCount = 0;
  if (!dashReadVarInt(p, end, vinCount)) return {};
  ...
  for (uint64_t i = 0; i < vinCount; ++i) {
    ...
    if (!dashReadVarInt(p, end, scriptSigLen)) return {};
    ...
    while (sp < scriptSigEnd) {
      uint8_t opcode = *sp;
      if (opcode == DashOpCode::OP_TRUE || opcode == DashOpCode::OP_FALSE ||
          opcode == DashOpCode::OP_IF || opcode == DashOpCode::OP_ELSE ||
          opcode == DashOpCode::OP_ENDIF || opcode == DashOpCode::OP_DROP ||
          opcode == DashOpCode::OP_EQUAL || opcode == DashOpCode::OP_EQUALVERIFY ||
          opcode == DashOpCode::OP_CHECKSIG || opcode == DashOpCode::OP_CHECKLOCKTIMEVERIFY ||
          opcode == DashOpCode::OP_SHA256 || opcode == DashOpCode::OP_HASH160 ||
          opcode == DashOpCode::OP_DUP) {
        ++sp;
        continue;
      }

      uint64_t pushLen = 0;
      if (opcode == DashOpCode::OP_PUSHDATA1) {
        ++sp;
...
// =============================================================================
// WIF decode and Dash signing helpers
// =============================================================================

bool DashHtlcScript::wifToPrivKey(const std::string& wif,
...
std::vector<uint8_t> DashHtlcScript::signInput(
