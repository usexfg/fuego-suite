...
std::string DashRpcClient::base64Encode(const std::string& input) {
  std::string out;
...
DashRpcClient::DashRpcClient(const std::string& host, uint16_t port,
...
std::string DashRpcClient::httpPost(const std::string& body) {
  int sock = socket(AF_INET, SOCK_STREAM, 0);
  if (sock < 0) {
    throw std::runtime_error("DashRpcClient: failed to create socket");
  }
  ...
  if (gai != 0) {
    close(sock);
    throw std::runtime_error("DashRpcClient: failed to resolve host: " + m_host);
  }
  ...
  if (ret < 0) {
    close(sock);
    throw std::runtime_error("DashRpcClient: failed to connect to " + m_host + ":" + portStr);
  }
  ...
  if (sent < 0 || static_cast<size_t>(sent) != request.size()) {
    close(sock);
    throw std::runtime_error("DashRpcClient: failed to send HTTP request");
  }
  ...
  if (headerEnd == std::string::npos) {
    throw std::runtime_error("DashRpcClient: malformed HTTP response");
  }
...
std::string DashRpcClient::rpcCall(const std::string& method, const std::string& params) {
  // Bitcoin JSON-RPC envelope (Dash Core speaks the same protocol)
...
bool DashRpcClient::getBlockCount(uint64_t& height) {
  try {
...
bool DashRpcClient::getTransaction(const std::string& txid, DashTxInfo& info) {
  try {
...
bool DashRpcClient::getBalance(const std::string& address, uint64_t& duffs) {
  try {
    // Use listunspent filtered by address, then sum up
    std::vector<DashUtxo> utxos;
...
bool DashRpcClient::listUnspent(const std::string& address, std::vector<DashUtxo>& utxos) {
  try {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      ...
      // Bitcoin RPC returns "amount" as a floating-point DASH value (e.g., 0.001).
      // Convert to duffs by multiplying by 1e8 and rounding.
      if (item.contains("amount")) {
        const auto& amtVal = item("amount");
        if (amtVal.isReal()) {
          double dash = amtVal.getReal();
          utxo.duffs = static_cast<uint64_t>(dash * 100000000.0 + 0.5);
        } else if (amtVal.isInteger()) {
          int64_t raw = amtVal.getInteger();
          utxo.duffs = (raw >= 0) ? static_cast<uint64_t>(raw) * 100000000ULL : 0;
        } else {
          utxo.duffs = 0;
        }
      } else {
...
bool DashRpcClient::getAddressPubkey(const std::string& address, std::string& pubkeyHex) {
  try {
...
bool DashRpcClient::estimateFeeDuffs(uint64_t& feeDuffs, int confTarget) {
  // Floor: 1000 duffs (previous hardcode). Prefer estimatesmartfee when available.
  feeDuffs = 1000;
  try {
    ...
    double dashFee = dashPerKb * 0.25;
    uint64_t est = static_cast<uint64_t>(dashFee * 1e8 + 0.5);
    if (est > feeDuffs) feeDuffs = est;
...
bool DashRpcClient::getRawTransaction(const std::string& txid, std::string& rawTxHex) {
  try {
...
bool DashRpcClient::sendRawTransaction(const std::string& rawTxHex, std::string& txid) {
  try {
...
bool DashRpcClient::decodeRawTransaction(const std::string& rawTxHex, std::string& jsonResult) {
  try {
    std::string params = "[\"" + rawTxHex + "\"]";
...
bool DashRpcClient::validateAddress(const std::string& address, bool& isValid) {
  try {
    std::string params = "[\"" + address + "\"]";
...
bool DashRpcClient::importAddress(const std::string& address, const std::string& label, bool rescan) {
  try {
...
bool DashRpcClient::lockHtlc(const std::string& senderWif,
  ...
  if (recipientAddress.size() == 66) {
    auto bytes = DashHtlcScript::hexToBytes(recipientAddress);
    if (bytes.size() == 33 && (bytes[0] == 0x02 || bytes[0] == 0x03)) {
      recipientPubKey = bytes;
    }
  }

  auto hashLockBytes = DashHtlcScript::hexToBytes(hashLockSha256Hex);
  if (hashLockBytes.size() != 32) return false;

  // Build redeem script and P2SH address.
  auto redeemScript = DashHtlcScript::createRedeemScript(
      hashLockBytes, recipientPubKey, senderPubKey, timeoutBlock);
  redeemScriptHex = DashHtlcScript::bytesToHex(redeemScript);
  std::string htlcAddress = DashHtlcScript::computeP2shAddress(redeemScript, m_testnet);
  ...
  // Fund the HTLC address by sending amountDuffs to it.
  // Use the node wallet's sendtoaddress RPC — this handles UTXO selection.
  std::string params = "[\"" + htlcAddress + "\"," +
                       std::to_string(static_cast<double>(amountDuffs) / 1e8) + "]";
  try {
...
bool DashRpcClient::verifyLock(const std::string& htlcAddress,
                               uint64_t expectedDuffs,
                               uint32_t minConfirms) {
  std::vector<DashUtxo> utxos;
  if (!listUnspent(htlcAddress, utxos)) return false;

  for (const auto& utxo : utxos) {
    if (utxo.duffs >= expectedDuffs &&
...
bool DashRpcClient::claim(const std::string& claimerWif,
  ...
  if (!DashHtlcScript::wifToPrivKey(claimerWif, privKey)) return false;

  auto redeemScript = DashHtlcScript::hexToBytes(redeemScriptHex);
  auto preimage     = DashHtlcScript::hexToBytes(preimageHex);

  // Build output script (P2PKH to destAddress).
  uint8_t addrVersion = 0;
  std::vector<uint8_t> pubKeyHash;
  if (!DashHtlcScript::base58CheckDecode(destAddress, addrVersion, pubKeyHash)) return false;
  auto outputScript = DashHtlcScript::buildP2pkhScriptPubKey(pubKeyHash);

  uint64_t fee = 1000;
  estimateFeeDuffs(fee, /*confTarget=*/2);
  ...
  // Sign using the pre-SegWit legacy sighash (Dash is NOT SegWit).
  auto der = DashHtlcScript::signInput(privKey, /*version=*/1, /*locktime=*/0,
                                       nSequence,
                                       htlcTxid, htlcVout,
  ...
  auto scriptSig = DashHtlcScript::createClaimScriptSig(der, preimage, redeemScript);

  // Build raw tx and broadcast.
  auto rawTx = DashHtlcScript::buildRawTransaction(
      htlcTxid, htlcVout, htlcAmount,
      scriptSig, destAddress, outputAmount, /*nLocktime=*/0);

  std::string txHex = DashHtlcScript::bytesToHex(rawTx);
  return sendRawTransaction(txHex, claimTxId);
}

bool DashRpcClient::refundHtlc(const std::string& senderWif,
                               const std::string& htlcTxid,
                               uint32_t htlcVout,
                               uint64_t htlcAmount,
                               const std::string& redeemScriptHex,
  ...
  std::array<uint8_t, 32> privKey{};
  if (!DashHtlcScript::wifToPrivKey(senderWif, privKey)) return false;
  ...
  auto der = DashHtlcScript::signInput(privKey, /*version=*/1, nLocktime,
  ...
  auto scriptSig = DashHtlcScript::createRefundScriptSig(der, redeemScript);
...
bool DashRpcClient::verifyMessage(const std::string& address, const std::string& signature,
