...
struct DashUtxo {
  std::string txid;
  uint32_t vout;
  uint64_t duffs;  // Dash satoshi-equivalent units (1e8 per DASH)
...
class DashRpcClient {
public:
  DashRpcClient(const std::string& host, uint16_t port,
                const std::string& rpcUser, const std::string& rpcPassword,
                bool testnet = false);

  // Basic queries
  bool getBlockCount(uint64_t& height);
  bool getTransaction(const std::string& txid, DashTxInfo& info);
  bool getBalance(const std::string& address, uint64_t& duffs);

  // UTXO queries
  bool listUnspent(const std::string& address, std::vector<DashUtxo>& utxos);
  ...
  // Fee estimation (duffs). Uses estimatesmartfee; falls back to floor.
  bool estimateFeeDuffs(uint64_t& feeDuffs, int confTarget = 2);
  ...
  // amountDuffs:       DASH to lock (1e8 units).
  ...
  // Claim DASH from an HTLC by revealing the preimage (adaptor secret).
  ...
  // Refund DASH from an HTLC after the timeout has elapsed.
