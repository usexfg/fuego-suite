...
// Dogecoin HTLC script (P2SH, pre-SegWit legacy sighash).
//
// Dogecoin is a Bitcoin 0.10-era fork with NO SegWit support. Unlike
// BTC/LTC/BCH/KMD (which use BIP143), Doge transactions are signed with the
// original legacy (pre-SegWit) sighash algorithm. Addresses use:
//   P2PKH version: 0x1E (mainnet), 0x71 (testnet)
//   P2SH  version: 0x16 (mainnet), 0xC4 (testnet)
//   WIF   version: 0x9E (mainnet), 0xF1 (testnet)
class DogeHtlcScript {
public:
  ...
  // Doge P2SH address = Base58Check(0x16 || RIPEMD160(SHA256(redeemScript)))
  // For Doge testnet: prefix 0xC4
  ...
  // WIF to private key (32 bytes). Doge WIF version: 0x9E (mainnet)
  static bool wifToPrivKey(const std::string& wif,
                           std::array<uint8_t, 32>& privKey);

  // Sign a Doge P2SH input using the legacy pre-SegWit sighash (SIGHASH_ALL=0x01).
