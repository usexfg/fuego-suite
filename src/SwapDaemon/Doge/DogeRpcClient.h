...
struct DogeUtxo {
  std::string txid;
  uint32_t vout;
  uint64_t koinu;  // Dogecoin satoshi-equivalent units (1e8 per DOGE)
...
class DogeRpcClient {
public:
  DogeRpcClient(const std::string& host, uint16_t port,
                const std::string& rpcUser, const std::string& rpcPassword);

  // Basic queries
  bool getBlockCount(uint64_t& height);
  bool getTransaction(const std::string& txid, DogeTxInfo& info);
  bool getBalance(const std::string& address, uint64_t& koinu);

  // UTXO queries
  bool listUnspent(const std::string& address, std::vector<DogeUtxo>& utxos);
  ...
  // Fee estimation (koinu). Uses estimatesmartfee; falls back to floor.
  bool estimateFeeKoinu(uint64_t& feeKoinu, int confTarget = 2);
  ...
  // senderWif:         WIF-encoded private key of the sender (mainnet prefix 0x9E).
  // timeoutBlock:      block height after which refund is valid.
  // amountKoinu:       DOGE to lock (1e8 units).
  ...
  // Claim DOGE from an HTLC by revealing the preimage (adaptor secret).
  ...
  // Refund DOGE from an HTLC after the timeout has elapsed.
