...
class KmdRpcClient {
public:
  KmdRpcClient(const std::string& host, uint16_t port,
               const std::string& rpcUser, const std::string& rpcPassword);

  // Basic queries
  bool getBlockCount(uint64_t& height);
  bool getTransaction(const std::string& txid, KmdTxInfo& info);
  bool getBalance(const std::string& address, uint64_t& satoshis);

  // UTXO queries
  bool listUnspent(const std::string& address, std::vector<KmdUtxo>& utxos);
  ...
  // HTLC operations
  //
  // Lock KMD in a P2SH HTLC.
  // hashLockSha256Hex: 64-char hex of SHA256(adaptor_secret) - NOT RIPEMD160.
  // recipientAddress: 66-char hex compressed pubkey of the recipient.
  // senderWif:        WIF-encoded private key of the sender (version 0xBC).
  // timeoutBlock:     block height after which refund is valid.
  // amountSatoshis:   KMD to lock.
  ...
  // Claim KMD from an HTLC by revealing the preimage (adaptor secret).
  ...
  // destAddress:    where to send the claimed KMD.
  ...
  // Refund KMD from an HTLC after the timeout has elapsed.
  ...
  // destAddress:    where to send the refunded KMD.
