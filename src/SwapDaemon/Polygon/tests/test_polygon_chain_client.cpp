...
#include "SwapDaemon/Polygon/PolygonChainClient.h"
...
static void test_chainName() {
  std::cout << "test_chainName..." << std::endl;

  auto rpc = std::make_unique<EthRpcClient>("127.0.0.1", 1);
  PolygonChainClient client(std::move(rpc), "0x0000000000000000000000000000000000000001");

  assert(client.chainName() == "POLYGON");
...
static void test_lock_error_contains_chain_name() {
  ...
  assert(result.error.find("POLYGON") != std::string::npos);
...
int main() {
  std::cout << "=== PolygonChainClient Tests ===" << std::endl;
