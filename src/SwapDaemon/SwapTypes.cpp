...
bool swapPairFromString(const std::string& s, SwapPair& out) {
  ...
  if (iequal(p, "PLS", n))     { out = SwapPair::PULSECHAIN; return true; }  // native coin ticker
  // Legacy spellings: the chain was previously misnamed after PulseX, the DEX
  // that runs on it. Still accepted so existing offers/configs keep resolving.
  if (iequal(p, "PULSEX", n))  { out = SwapPair::PULSECHAIN; return true; }
  if (iequal(p, "PULS", n))    { out = SwapPair::PULSECHAIN; return true; }
