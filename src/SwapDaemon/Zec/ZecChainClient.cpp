...
ZecChainClient::ZecChainClient(std::unique_ptr<ZecRpcClient> rpc, const std::string& wif)
  : m_rpc(std::move(rpc)), m_wif(wif) {}

ChainClientResult ZecChainClient::lock(const SwapParams& params) {
  if (!m_rpc)
    return ChainClientResult::fail("ZEC lock: RPC client not available");

  // Hashlock: Alice-locks uses published H(t)=SHA256(t); Bob-with-secret can derive.
  std::string hashHex;
  if (!isZeroSecret(params.adaptorSecret)) {
    hashHex = zecHashLockHex(params.adaptorSecret);
  } else {
    bool nonzero = false;
    for (size_t i = 0; i < sizeof(params.hashLock); ++i)
      if (reinterpret_cast<const uint8_t*>(&params.hashLock)[i]) { nonzero = true; break; }
    if (!nonzero)
      return ChainClientResult::fail("ZEC lock: no adaptor secret or hashLock (need H(t) from Bob)");
  ...
  if (recipientKey.empty() && params.ctrAddress.size() == 66) {
    try {
      auto bytes = ZecHtlcScript::hexToBytes(params.ctrAddress);
  ...
  if (recipientKey.size() != 66)
    return ChainClientResult::fail(
        "ZEC lock: ctrPubKey must be 33-byte compressed pubkey hex (66 chars). "
        "A Zec address cannot be inverted to a pubkey; set ctrPubKey or use a wallet-known address.");
  ...
  bool ok = m_rpc->lockHtlc(
      ...
      lockTxId,
      redeemScriptHex);
  if (!ok) return ChainClientResult::fail("ZEC lockHtlc failed");
  return ChainClientResult::okWithState(lockTxId, redeemScriptHex);
}

ChainClientResult ZecChainClient::verifyLock(const SwapParams& params) {
  if (!m_rpc)
    return ChainClientResult::fail("ZEC verifyLock: no RPC client available");
  ...
  if (!params.chainState.empty()) {
    auto redeem = ZecHtlcScript::hexToBytes(params.chainState);
    if (redeem.empty())
      return ChainClientResult::fail("ZEC verifyLock: invalid redeem script in chainState");
    htlcAddress = ZecHtlcScript::computeP2shAddress(redeem, /*testnet=*/false);
    ...
    return ChainClientResult::fail(
        "ZEC verifyLock: need chainState (redeem script) or P2SH address — "
        "cannot listunspent by txid alone");
  }

  bool ok = m_rpc->verifyLock(htlcAddress, params.ctrAmount);
  if (!ok) return ChainClientResult::fail("ZEC lock not verified at " + htlcAddress);
  return ChainClientResult::ok(params.ctrLockTxId);
}

ChainClientResult ZecChainClient::claim(const SwapParams& params) {
  if (!m_rpc)
    return ChainClientResult::fail("ZEC claim: no RPC client available");
  ...
  bool ok = m_rpc->claim(
      ...
      params.ctrAddress,
      claimTxId);
  if (!ok) return ChainClientResult::fail("ZEC claim failed");
  return ChainClientResult::ok(claimTxId);
}

ChainClientResult ZecChainClient::refund(const SwapParams& params) {
  if (!m_rpc)
    return ChainClientResult::fail("ZEC refund: no RPC client available");
  ...
  bool ok = m_rpc->refundHtlc(
      ...
      params.ctrAddress,
      refundTxId);
  if (!ok) return ChainClientResult::fail("ZEC refundHtlc failed");
  return ChainClientResult::ok(refundTxId);
}

ChainClientResult ZecChainClient::verifyReserveProof(const std::string& expectedMessage,
    uint64_t minAmount, const std::string& proof) {
  if (!m_rpc)
    return ChainClientResult::fail("ZEC verifyReserveProof: RPC client not available");

  size_t c1 = proof.find(':');
  size_t c2 = proof.find(':', c1 + 1);
  if (c1 == std::string::npos || c2 == std::string::npos)
    return ChainClientResult::fail("ZEC reserve proof: invalid format (expected address:signature:message)");
  ...
  if (!m_rpc->verifyMessage(address, signature, message, sigValid))
    return ChainClientResult::fail("ZEC reserve proof: verifymessage RPC failed");
  if (!sigValid)
    return ChainClientResult::fail("ZEC reserve proof: invalid signature");

  if (!expectedMessage.empty() && message != expectedMessage)
    return ChainClientResult::fail("ZEC reserve proof: message not bound to this offer");

  uint64_t balance = 0;
  if (!m_rpc->getBalance(address, balance))
    return ChainClientResult::fail("ZEC reserve proof: balance check RPC failed");
  if (balance < minAmount)
    return ChainClientResult::fail("ZEC reserve proof: insufficient balance (" +
                                   std::to_string(balance) + " < " + std::to_string(minAmount) + ")");

  return ChainClientResult::ok(address);
}

bool ZecChainClient::getCurrentHeight(uint64_t& height) {
  if (m_rpc) {
...
ChainClientResult ZecChainClient::getTransactionDetails(const std::string& txId,
                                                         ChainClientResult& result) {
  if (!m_rpc) {
    result = ChainClientResult::fail("ZEC: no RPC client available");
    return result;
  }

  ZecTxInfo txInfo;
  if (!m_rpc->getTransaction(txId, txInfo)) {
    result = ChainClientResult::fail("ZEC RPC: gettransaction failed for " + txId);
    return result;
  }
...
std::string ZecChainClient::extractSecret(const std::string& spendingTxid,
                                           const std::string& htlcRedeemScriptHex) {
  std::vector<uint8_t> redeemScript = ZecHtlcScript::hexToBytes(htlcRedeemScriptHex);
  std::vector<uint8_t> p2shScriptPubKey = ZecHtlcScript::redeemScriptToP2shScriptPubKey(redeemScript);
  ...
  std::vector<uint8_t> rawTx = ZecHtlcScript::hexToBytes(rawTxHex);
  std::vector<uint8_t> preimage = ZecHtlcScript::parseClaimPreimage(rawTx, p2shScriptPubKey);
  if (preimage.empty()) {
    return {};
  }

  return ZecHtlcScript::bytesToHex(preimage);
}

std::string ZecChainClient::tryExtractClaimedSecret(const SwapParams& params) {
