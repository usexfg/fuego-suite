...
std::string ZecRpcClient::base64Encode(const std::string& input) {
  std::string out;
...
ZecRpcClient::ZecRpcClient(const std::string& host, uint16_t port,
...
std::string ZecRpcClient::httpPost(const std::string& body) {
  int sock = socket(AF_INET, SOCK_STREAM, 0);
  if (sock < 0) {
    throw std::runtime_error("ZecRpcClient: failed to create socket");
  }
  ...
  if (gai != 0) {
    close(sock);
    throw std::runtime_error("ZecRpcClient: failed to resolve host: " + m_host);
  }
  ...
  if (ret < 0) {
    close(sock);
    throw std::runtime_error("ZecRpcClient: failed to connect to " + m_host + ":" + portStr);
  }
  ...
  if (sent < 0 || static_cast<size_t>(sent) != request.size()) {
    close(sock);
    throw std::runtime_error("ZecRpcClient: failed to send HTTP request");
  }
  ...
  if (headerEnd == std::string::npos) {
    throw std::runtime_error("ZecRpcClient: malformed HTTP response");
  }
...
std::string ZecRpcClient::rpcCall(const std::string& method, const std::string& params) {
  // Bitcoin JSON-RPC envelope (Zec Core speaks the same protocol)
...
bool ZecRpcClient::getBlockCount(uint64_t& height) {
  try {
...
bool ZecRpcClient::getTransaction(const std::string& txid, ZecTxInfo& info) {
  try {
...
bool ZecRpcClient::getBalance(const std::string& address, uint64_t& zats) {
  try {
    // Use listunspent filtered by address, then sum up
    std::vector<ZecUtxo> utxos;
...
bool ZecRpcClient::listUnspent(const std::string& address, std::vector<ZecUtxo>& utxos) {
  try {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      ...
      // Bitcoin RPC returns "amount" as a floating-point ZEC value (e.g., 0.001).
      // Convert to zats by multiplying by 1e8 and rounding.
      if (item.contains("amount")) {
        const auto& amtVal = item("amount");
        if (amtVal.isReal()) {
          double zec = amtVal.getReal();
          utxo.zats = static_cast<uint64_t>(zec * 100000000.0 + 0.5);
        } else if (amtVal.isInteger()) {
          int64_t raw = amtVal.getInteger();
          utxo.zats = (raw >= 0) ? static_cast<uint64_t>(raw) * 100000000ULL : 0;
        } else {
          utxo.zats = 0;
        }
      } else {
...
bool ZecRpcClient::getAddressPubkey(const std::string& address, std::string& pubkeyHex) {
  try {
...
bool ZecRpcClient::estimateFeeZats(uint64_t& feeZats, int confTarget) {
  // Floor: 1000 zats (previous hardcode). Prefer estimatesmartfee when available.
  feeZats = 1000;
  try {
    ...
    double zecFee = zecPerKb * 0.25;
    uint64_t est = static_cast<uint64_t>(zecFee * 1e8 + 0.5);
    if (est > feeZats) feeZats = est;
...
bool ZecRpcClient::getRawTransaction(const std::string& txid, std::string& rawTxHex) {
  try {
...
bool ZecRpcClient::sendRawTransaction(const std::string& rawTxHex, std::string& txid) {
  try {
...
bool ZecRpcClient::decodeRawTransaction(const std::string& rawTxHex, std::string& jsonResult) {
  try {
    std::string params = "[\"" + rawTxHex + "\"]";
...
bool ZecRpcClient::validateAddress(const std::string& address, bool& isValid) {
  try {
    std::string params = "[\"" + address + "\"]";
...
bool ZecRpcClient::importAddress(const std::string& address, const std::string& label, bool rescan) {
  try {
...
bool ZecRpcClient::lockHtlc(const std::string& senderWif,
  ...
  if (!ZecHtlcScript::wifToPrivKey(senderWif, senderPrivKey)) return false;
  ...
  if (recipientAddress.size() == 66) {
    auto bytes = ZecHtlcScript::hexToBytes(recipientAddress);
    if (bytes.size() == 33 && (bytes[0] == 0x02 || bytes[0] == 0x03)) {
      recipientPubKey = bytes;
    }
  }

  auto hashLockBytes = ZecHtlcScript::hexToBytes(hashLockSha256Hex);
  if (hashLockBytes.size() != 32) return false;

  // Build redeem script and P2SH address.
  auto redeemScript = ZecHtlcScript::createRedeemScript(
      hashLockBytes, recipientPubKey, senderPubKey, timeoutBlock);
  redeemScriptHex = ZecHtlcScript::bytesToHex(redeemScript);
  std::string htlcAddress = ZecHtlcScript::computeP2shAddress(redeemScript, m_testnet);
  ...
  // Fund the HTLC address by sending amountZats to it.
  // Use the node wallet's sendtoaddress RPC — this handles UTXO selection.
  std::string params = "[\"" + htlcAddress + "\"," +
                       std::to_string(static_cast<double>(amountZats) / 1e8) + "]";
  try {
...
bool ZecRpcClient::verifyLock(const std::string& htlcAddress,
...
bool ZecRpcClient::claim(const std::string& claimerWif,
  ...
  if (!ZecHtlcScript::wifToPrivKey(claimerWif, privKey)) return false;

  auto redeemScript = ZecHtlcScript::hexToBytes(redeemScriptHex);
  auto preimage     = ZecHtlcScript::hexToBytes(preimageHex);

  // Build output script (P2PKH to destAddress).
  std::vector<uint8_t> addrVersion;
  std::vector<uint8_t> pubKeyHash;
  if (!ZecHtlcScript::base58CheckDecode(destAddress, addrVersion, pubKeyHash)) return false;
  auto outputScript = ZecHtlcScript::buildP2pkhScriptPubKey(pubKeyHash);

  uint64_t fee = 1000;
  estimateFeeZats(fee, /*confTarget=*/2);
  ...
  // Sign using the pre-SegWit legacy sighash (Zec is NOT SegWit).
  auto der = ZecHtlcScript::signInput(privKey, /*version=*/4, /*locktime=*/0,
                                       nSequence,
                                       htlcTxid, htlcVout,
  ...
  auto scriptSig = ZecHtlcScript::createClaimScriptSig(der, preimage, redeemScript);

  // Build raw tx and broadcast.
  auto rawTx = ZecHtlcScript::buildRawTransaction(
      htlcTxid, htlcVout, htlcAmount,
      scriptSig, destAddress, outputAmount, /*nLocktime=*/0);

  std::string txHex = ZecHtlcScript::bytesToHex(rawTx);
  return sendRawTransaction(txHex, claimTxId);
}

bool ZecRpcClient::refundHtlc(const std::string& senderWif,
                               const std::string& htlcTxid,
                               uint32_t htlcVout,
                               uint64_t htlcAmount,
                               const std::string& redeemScriptHex,
  ...
  std::array<uint8_t, 32> privKey{};
  if (!ZecHtlcScript::wifToPrivKey(senderWif, privKey)) return false;
  ...
  auto der = ZecHtlcScript::signInput(privKey, /*version=*/4, nLocktime,
  ...
  auto scriptSig = ZecHtlcScript::createRefundScriptSig(der, redeemScript);
...
bool ZecRpcClient::verifyMessage(const std::string& address, const std::string& signature,
