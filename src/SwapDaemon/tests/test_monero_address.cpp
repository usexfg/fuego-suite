...
int main() {
  ...
  // 4. Ground truth: a REAL Monero mainnet address (generated by
  //    monero-wallet-cli v0.18.4.6) must decode and re-encode byte-identically
  //    through MoneroAddress — proves compatibility with actual Monero, not
  //    just self-consistency.
  {
    const std::string REAL =
        "44nkoAFptrPVAv92eAr4tiPEqGgQE15v4EuQ9YCzFVD4EpjPMxLC3AZ6CLcZZhRKspBkyDyPkC5XjhQTSy2azsxcVgyhXPW";
    uint64_t rtag = 0;
    std::string rdata;
    bool rok = Tools::Base58::decode_addr(REAL, rtag, rdata);
    assert(rok && "real Monero address must decode");
    assert(rtag == MoneroAddress::MAINNET && "real address tag == 18");
    assert(rdata.size() == 64 && "real address payload = spend||view");
    const uint8_t* rd = reinterpret_cast<const uint8_t*>(rdata.data());
    std::vector<uint8_t> rspend(rd, rd + 32), rview(rd + 32, rd + 64);
    assert(MoneroAddress::encode(rspend, rview, MoneroAddress::MAINNET) == REAL &&
           "MoneroAddress must re-encode a real Monero address identically");
    std::cout << "  [4] real monero-wallet-cli address re-encodes identically\n";
  }
