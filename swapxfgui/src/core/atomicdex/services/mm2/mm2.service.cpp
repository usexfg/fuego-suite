...
namespace atomic_dex
{
    ...
    void mm2_service::spawn_mm2_instance(std::string wallet_name, std::string passphrase, bool with_pin_cfg, std::string rpcpass)
    {
        ...
        if (!started)
        {
            SPDLOG_ERROR("Couldn't start mm2/fuego-swapd daemon");
        ...
        m_mm2_init_thread = std::thread(
            [this, mm2_cfg_path]()
            {
                ...
                while (not check_mm2_alive())
                {
                    ...
                    if (nb_try == 30)
                    {
                        SPDLOG_ERROR("Daemon not started correctly");
                ...
                std::filesystem::remove(mm2_cfg_path);
                SPDLOG_INFO("fuego-swapd daemon is initialized");
    ...
    namespace
    {
        ...
        auto
        fuego_rpc_call(typename RpcType::expected_request_type req) -> std::optional<typename RpcType::expected_result_type>
        {
            try
            {
                auto client = std::make_unique<web::http::client::http_client>(FROM_STD_STR(atomic_dex::g_dex_rpc));
                nlohmann::json json_data;
                json_data["method"]  = RpcType::endpoint;
                json_data["userpass"] = mm2::get_rpc_password();
                json_data["mmrpc"]   = "2.0";
                json_data["id"]      = 42;

                nlohmann::json params;
                nlohmann::to_json(params, req);
                ...
                if (body_json.contains("error") && !body_json.at("error").is_null())
                {
                    SPDLOG_ERROR("fuego RPC {} error: {}", RpcType::endpoint, body_json.at("error").dump());
                    return std::nullopt;
                }

                if constexpr (std::is_same_v<typename RpcType::expected_result_type, nlohmann::json>)
                {
                    return body_json.at("result");
                }
                else
                {
                    typename RpcType::expected_result_type result;
                    mm2::from_json(body_json.at("result"), result);
                    return result;
                }
            }
            catch (const std::exception& ex)
            {
                SPDLOG_ERROR("fuego RPC {} exception: {}", RpcType::endpoint, ex.what());
                return std::nullopt;
            }
        }
    } // namespace
    ...
    void
    mm2_service::start_fuego_ws()
    {
        if (m_fuego_ws)
            return;
        fuego::fuego_ws_config cfg;
        cfg.on_block = [this](int64_t height, const std::string& coin, const std::string& hash)
        {
            SPDLOG_DEBUG("fuego WS block: {} on {}", height, coin);
            dispatcher_.trigger<force_update_providers>();
        };
        cfg.on_swap_update = [this](const std::string& uuid, const std::string& status, const nlohmann::json& details)
        {
            SPDLOG_DEBUG("fuego WS swap update: {} -> {}", uuid, status);
            batch_fetch_orders_and_swap();
        };
        m_fuego_ws = std::make_unique<fuego::fuego_ws_client>(std::move(cfg));
        m_fuego_ws->start();
    }

    void
    mm2_service::stop_fuego_ws()
    {
        if (m_fuego_ws)
        {
            m_fuego_ws->stop();
    ...
    fuego::fuego_balance_result
    mm2_service::fuego_get_balance(const std::string& coin)
    {
        fuego::fuego_balance_request req{.coin = coin};
        auto result = fuego_rpc_call<fuego::fuego_balance_rpc>(std::move(req));
        return result.value_or(fuego::fuego_balance_result{});
    }

    fuego::fuego_send_result
    mm2_service::fuego_send(const std::string& coin, const std::string& to, const std::string& amount, std::optional<std::string> memo)
    {
        fuego::fuego_send_request req{.coin = coin, .to = to, .amount = amount, .memo = std::move(memo)};
        auto result = fuego_rpc_call<fuego::fuego_send_rpc>(std::move(req));
        return result.value_or(fuego::fuego_send_result{});
    }

    fuego::fuego_burn2mint_result
    mm2_service::fuego_burn2mint(const std::string& coin, const std::string& amount, std::optional<std::string> dest)
    {
        fuego::fuego_burn2mint_request req{.coin = coin, .amount = amount, .destination_address = std::move(dest)};
        auto result = fuego_rpc_call<fuego::fuego_burn2mint_rpc>(std::move(req));
        return result.value_or(fuego::fuego_burn2mint_result{});
    }

    fuego::fuego_alias_result
    mm2_service::fuego_resolve_alias(const std::string& name)
    {
        fuego::fuego_alias_request req{.name = name};
        auto result = fuego_rpc_call<fuego::fuego_alias_rpc>(std::move(req));
        return result.value_or(fuego::fuego_alias_result{});
    }

    fuego::fuego_alias_result
    mm2_service::fuego_lookup_address(const std::string& address)
    {
        fuego::fuego_alias_request req{.address = address};
        auto result = fuego_rpc_call<fuego::fuego_alias_rpc>(std::move(req));
        return result.value_or(fuego::fuego_alias_result{});
    }

    std::vector<fuego::fuego_activity_entry>
    mm2_service::fuego_get_activity(const std::string& coin, std::optional<std::size_t> limit, std::optional<std::size_t> offset)
    {
        fuego::fuego_activity_request req{.coin = coin, .limit = limit, .offset = offset};
        auto result = fuego_rpc_call<fuego::fuego_activity_rpc>(std::move(req));
        if (result.has_value())
            return std::move(result->entries);
        return {};
    }

    fuego::hearth_pool_stats_result
    mm2_service::fuego_get_pool_stats(const std::string& coin_a, const std::string& coin_b)
    {
        fuego::hearth_pool_stats_request req{.coin_a = coin_a, .coin_b = coin_b};
        auto result = fuego_rpc_call<fuego::hearth_pool_stats_rpc>(std::move(req));
        return result.value_or(fuego::hearth_pool_stats_result{});
    }

    fuego::hearth_quote_result
    mm2_service::fuego_get_quote(const std::string& coin_a, const std::string& coin_b, const std::string& amount, bool exact_input)
    {
        fuego::hearth_quote_request req{.coin_a = coin_a, .coin_b = coin_b, .amount = amount, .exact_input = exact_input};
        auto result = fuego_rpc_call<fuego::hearth_quote_rpc>(std::move(req));
        return result.value_or(fuego::hearth_quote_result{});
    }

    fuego::hearth_swap_execute_result
    mm2_service::fuego_swap_execute(const std::string& coin_a, const std::string& coin_b, const std::string& amount, const std::string& min_received, const std::string& deadline)
    {
        fuego::hearth_swap_execute_request req{.coin_a = coin_a, .coin_b = coin_b, .amount = amount, .min_received = min_received, .deadline = deadline};
        auto result = fuego_rpc_call<fuego::hearth_swap_execute_rpc>(std::move(req));
        return result.value_or(fuego::hearth_swap_execute_result{});
    }

    std::vector<fuego::cd_info>
    mm2_service::fuego_list_cds()
    {
        auto result = fuego_rpc_call<fuego::cd_list_rpc>(nlohmann::json::object());
        if (result.has_value())
            return std::move(result->cds);
        return {};
    }

    fuego::cd_create_result
    mm2_service::fuego_create_cd(const std::string& coin, const std::string& amount, std::optional<int64_t> duration)
    {
        fuego::cd_create_request req{.coin = coin, .amount = amount, .duration_blocks = duration};
        auto result = fuego_rpc_call<fuego::cd_create_rpc>(std::move(req));
        return result.value_or(fuego::cd_create_result{});
    }

    fuego::cd_claim_result
    mm2_service::fuego_claim_cd(const std::string& cd_id)
    {
        fuego::cd_claim_request req{.cd_id = cd_id};
        auto result = fuego_rpc_call<fuego::cd_claim_rpc>(std::move(req));
        return result.value_or(fuego::cd_claim_result{});
    }
