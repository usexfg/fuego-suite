   ...
   class ENTT_API mm2_service final : public ag::ecs::pre_update_system<mm2_service>
   {
     ...
     private:
       ...
       t_coins                  m_activation_queue;
        ...
        //! Fuego WebSocket client
        std::unique_ptr<fuego::fuego_ws_client> m_fuego_ws;
     ...
     public:
       ...
       void on_gui_leave_trading(const gui_leave_trading& evt);
        ...
        //! Fuego daemon management
        void start_fuego_ws();
        void stop_fuego_ws();

        //! Fuego RPC helpers (synchronous wrappers)
        fuego::fuego_balance_result  fuego_get_balance(const std::string& coin);
        fuego::fuego_send_result     fuego_send(const std::string& coin, const std::string& to, const std::string& amount, std::optional<std::string> memo = {});
        fuego::fuego_burn2mint_result fuego_burn2mint(const std::string& coin, const std::string& amount, std::optional<std::string> dest = {});
        fuego::fuego_alias_result    fuego_resolve_alias(const std::string& name);
        fuego::fuego_alias_result    fuego_lookup_address(const std::string& address);
        std::vector<fuego::fuego_activity_entry> fuego_get_activity(const std::string& coin, std::optional<std::size_t> limit = {}, std::optional<std::size_t> offset = {});

        //! Hearth RPC helpers
        fuego::hearth_pool_stats_result fuego_get_pool_stats(const std::string& coin_a, const std::string& coin_b);
        fuego::hearth_quote_result      fuego_get_quote(const std::string& coin_a, const std::string& coin_b, const std::string& amount, bool exact_input);
        fuego::hearth_swap_execute_result fuego_swap_execute(const std::string& coin_a, const std::string& coin_b, const std::string& amount, const std::string& min_received, const std::string& deadline);

        //! CD RPC helpers
        std::vector<fuego::cd_info> fuego_list_cds();
        fuego::cd_create_result     fuego_create_cd(const std::string& coin, const std::string& amount, std::optional<int64_t> duration = {});
        fuego::cd_claim_result      fuego_claim_cd(const std::string& cd_id);
