...
TEST(FixedPoint, one) {
  auto o = FixedPoint64::one();
  EXPECT_FALSE(o.isZero());
  EXPECT_TRUE(o.isPositive());
  EXPECT_EQ(o.toUint64(), 1u);
}
...
TEST(FixedPoint, negate) {
  ...
  EXPECT_EQ(neg3.toUint64(), (uint64_t)(-3));
}
...
TEST(FixedPoint, div_zero) {
  ...
  EXPECT_EQ(a.div(zero).toUint64(), 0u);
}
...
TEST(FixedPoint, deterministic_mul) {
  auto x = FixedPoint64::fromRatio(355, 113);
  auto y = FixedPoint64::fromRatio(22, 7);
  auto result = x.mul(y);
